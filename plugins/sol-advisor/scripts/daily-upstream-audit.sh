...
if [ -n "${SOL_ADVISOR_AUDIT_STATE_DIR-}" ]; then
  state_dir=$SOL_ADVISOR_AUDIT_STATE_DIR
elif [ -n "${CODEX_HOME-}" ]; then
  state_dir=$CODEX_HOME/state/sol-advisor
else
  [ -n "${HOME-}" ] || { printf '%s\n' 'ERROR: HOME is unset.' >&2; exit 1; }
  state_dir=$HOME/.codex/state/sol-advisor
fi
...
tmp_dir=$(mktemp -d "$tmp_base/sol-advisor-upstream-audit.XXXXXX") || exit 1
cleanup() {
  case "$tmp_dir" in
    "$tmp_base"/sol-advisor-upstream-audit.*) rm -rf "$tmp_dir" ;;
