...
Use these contracts with Codex Orchestration's namespaced, role-pinned native custom agents.
...
2. Require native exposure of the exact selected implementation type. Do not require
   ...
   fallback tier only when it is attempted, and check `codex_orchestration_sol_reviewer`
...
`codex_orchestration_terra_executive` with task name `terra_high_exec_<objective_slug>` and
...
The executive performs final review itself. Do not add `codex_orchestration_sol_reviewer`
...
~~~text
ROLE
Act as Codex Orchestration's score-selected implementation worker. Execute the supplied
