...
def main() -> int:
    ...
    require(
        router,
        (
            "CODEX_ORCHESTRATION_ROOT_CONTRACT", "0.9.0-native-spawn-v5",
            ...
            "Do not emit a separate classification message", "classification wait",
            "list_threads", "read_thread", "`LAST_TASK_CONTEXT` (max 6,000 characters",
            "Give only the implementer",
            "LAUNCH UX", "only pre-launch text is exactly `Orchestration: ON for this chat`",
            "emit no pre-launch commentary", "never narrate analysis",
            "classification, planning",
            "named child lane is the only startup progress indicator", "## Classification",
            "- Relationship: <New|Amend|Replace|Cancel>",
            "- Active objective: <concise objective>", "- Work class: <friendly class>",
            "- Complexity: <1.0-10.0 / 10>", "- Why: <brief reason>",
            "NATIVE COLLABORATION LAUNCH", "very next tool call must be the native",
            "collaboration `spawn_agent` tool itself", "directly in the collaboration namespace",
            "agent_type=<selected mapped custom agent>", "fork_turns=\"none\"",
            "message=packet", "luna_max_implementer_<objective_slug>",
            "terra_max_implementer_<objective_slug>", "sol_high_implementer_<objective_slug>",
            "result identifies the child by that task name", "visible model\nlane at creation time",
            ...
            "do not rename the\nchild afterward", "`ALL_TOOLS`", *HUMAN_ROUTES,
            "Never spawn a supervisor, reviewer, grader, classifier, or a", "second writer",
            "END_TO_END_WORK", "CLASSIFICATION=<exact Markdown>",
            "PRIOR_COMPLETED_RESULT=<TURN value>", "TASK_CONTEXT_BUNDLE=<TURN path>",
            ...
            "every request and substantive", "root-visible fact in chronological order",
            ...
            "Ground\ntruth and Source", "Missing/ambiguous identity",
            "capture screenshot", "Judge pass, fail, or blocked", "end without editing,",
            "`followup_task`", "primary content",
            "preserving delivered work/proof",
            "after the work account; never replace it.",
            "FINAL-REPORT VOICE", "User-facing report changes",
            ...
            "what happened, work done/found", "outcome,\ndecisive evidence",
            "links, limits, or open work", "except `## Next step`",
            "locally add only", "request a rewrite",
            "`None — no next step is needed.`", "No fixed headings or field list",
    ...
    forbid(
        ...
        (
            ...
            "codex_orchestration_terra_orchestrator", "fork_turns=1", "fork_turns=none",
            "multi_agent_v1__spawn_agent", "set_thread_title",
            "fork_context: false",
        ),
        "root coordinator",
    )
    ...
    for filename in (
        ...
        require(
            ...
            (
                ...
                "At startup, never fetch/pull", "`ls-remote`", "compare remote refs",
                "checkout cleanliness", "Inspect targeted local state only",
                "If there is no release, do no", "remote Git check",
                "implementation plus release", "implement and verify first",
                "relevant status/diff", "fetch the target remote once", "reconcile",
                "deployment-only work", "release-boundary check", "happens immediately",
                ...
                "Only the completed user report changes",
                "slightly less technical language", "lead with the outcome",
                "explain jargon briefly", "retain useful exact details",
                "Keep implementation,", "verification, and agent communication",
        ...
        forbid(
            ...
            (
                ...
                "## Continuity", "## Completed", "### Current state", "### Next step",
                "Before the first edit", "establish one synchronization baseline",
                "establish one fresh GitHub baseline at the release boundary",
            ),
            filename,
        )
