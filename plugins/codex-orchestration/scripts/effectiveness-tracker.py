#!/usr/bin/env python3
"""Track Codex Orchestration account usage and terminally completed routed tasks over time."""
...
def state_root(configured: str | None = None) -> Path:
    environment = os.environ.get("CODEX_ORCHESTRATION_USAGE_STATE_DIR")
    if not environment:
        # Compatibility fallback; new callers should use CODEX_ORCHESTRATION_*.
    ...
    try:
        send_message(
            process,
            {
                ...
                "params": {
                    "clientInfo": {
                        "name": "codex_orchestration_effectiveness_tracker",
                        "title": "Codex Orchestration Effectiveness Tracker",
                        "version": "1.0.0",
                    }
                },
            },
...
def capture_snapshot(
    ...
    snapshot = {
        ...
        "codex_orchestration_completion_metrics": completion_metrics(root),
    }
...
def print_report(baseline: dict[str, Any], latest: dict[str, Any]) -> None:
    ...
    print(f"Completed Codex Orchestration tasks: {completion_delta:,}")
    if task_token_record_delta > 0:
        print(f"Exact recorded task tokens: {task_token_delta:,}")
        print(
            "Average tokens per completed Codex Orchestration task: "
        ...
        print("Average tokens per completed Codex Orchestration task: n/a")
    print(
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--codex-bin",
        default=os.environ.get("CODEX_ORCHESTRATION_CODEX_BIN")
        or os.environ.get("SOL_ADVISOR_CODEX_BIN", "codex"),
    )
