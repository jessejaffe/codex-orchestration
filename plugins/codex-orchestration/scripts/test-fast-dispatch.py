...
def main() -> int:
    ...
    required = (
        ...
        "safe equivalent authenticated, local, staging, or rendered-artifact surface",
        "Alternatives exhausted",
        "ROOT_VERIFICATION_RECOVERY_REQUIRED",
        "do not add a role lane",
        "## Continuity",
        "## Completed",
        "### Current state",
        "### Recommendations",
        "### Next step",
        "Both current state and recommendations",
        "are required; neither may be omitted or artificially minimized",
        "replacement inputs or behavior",
        "valid only when supported by the findings",
        "None — <reason no further action",
        "Reject a bare `None` or invented follow-on work",
