#!/bin/sh
# Install the six Codex Orchestration 0.8.17 companion profiles and retire obsolete identities safely.
...
if [ "$check_only" -eq 1 ]; then
  ...
  printf '%s\n' 'CHECK PASSED: six 0.8.17 companion profiles are current and obsolete roles are absent.'
  exit 0
fi
...
printf '%s\n' 'INSTALL PASSED: six 0.8.17 companion profiles are current and obsolete identities were retired.'
