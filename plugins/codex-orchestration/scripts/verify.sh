...
for document in "$skill" "$contracts"; do
  ...
  grep -Fq 'agent_type: codex_orchestration_sol_high_implementer' "$document" || fail "missing Sol/High implementer spawn in $document"
...
grep -Fq 'first Codex Orchestration activation of each local calendar day' "$skill" || fail "skill omits once-daily activation boundary"
