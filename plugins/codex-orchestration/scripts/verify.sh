#!/bin/sh
# Verify the Codex Orchestration 0.11.1 release without network access.
...
[ "$(jq -r .version "$manifest")" = 0.11.1 ] || fail 'manifest version must be exactly 0.11.1'
printf '%s\n' "$(jq -r .version "$manifest")" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+$' ||
  fail 'manifest must use traditional semantic versioning without a cachebuster'
pass 'manifest uses traditional version 0.11.1'
...
grep -Fq "requires the traditional release version 0.11.1" "$script_dir/reinstall-plugin.sh" ||
  fail 'reinstaller does not enforce 0.11.1'
grep -Fq "grep -Eq '^0\\.11\\.1$'" "$script_dir/reinstall-plugin.sh" ||
  fail 'reinstaller version matcher is not pinned to 0.11.1'
...
if [ -f "$repo_readme" ] && [ ! -L "$repo_readme" ]; then
  for value in \
    ...
    'same implementer' \
    'emits the selected implementer first' \
    'model lane rather than the work class' \
    "classifier's concrete reason" \
    'ongoing task' \
    'waits until an agent' \
    'seven companion profiles' \
    'standard service tier' \
    'Terra / Extra High' \
    'versioned task-context bundle' \
    'same running implementer and supervisor' \
  ...
  pass '0.11.1 documentation'
else
  pass 'repository documentation is intentionally outside the installed plugin package'
fi

pass 'Codex Orchestration 0.11.1 release verification complete'
