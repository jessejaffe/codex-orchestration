#!/bin/sh
# Verify the Codex Orchestration 0.8.6 release without network access.
...
for name, pin in expected.items():
    ...
    if any(value in name for value in ("grader", "read-only", "supervisor", "executive")):
...
grep -Fq "requires the traditional release version 0.8.6" "$script_dir/reinstall-plugin.sh" ||
  fail 'reinstaller does not enforce 0.8.6'
...
if [ -f "$repo_readme" ] && [ ! -L "$repo_readme" ]; then
  ...
  pass '0.8.6 documentation'
else
  pass 'repository documentation is intentionally outside the installed plugin package'
fi

pass 'Codex Orchestration 0.8.6 release verification complete'
