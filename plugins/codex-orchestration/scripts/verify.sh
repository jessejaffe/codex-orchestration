#!/bin/sh
# Verify the Codex Orchestration 0.8.1 release without network access.
...
printf '%s\n' '# customized retired role' > "$target/codex-orchestration-terra-executive.toml"
retired_digest=$(shasum -a 256 "$target/codex-orchestration-terra-executive.toml" | awk '{print $1}')
if sh "$script_dir/install-agents.sh" --target-dir "$target" >/dev/null 2>&1; then
  fail 'agent installer deleted an unrecognized retired role'
fi
[ "$(shasum -a 256 "$target/codex-orchestration-terra-executive.toml" | awk '{print $1}')" = "$retired_digest" ] ||
  fail 'customized retired role changed during rejected migration'
pass 'conflict-safe eight-role installer behavior'

grep -Fq "requires the traditional release version 0.8.1" "$script_dir/reinstall-plugin.sh" ||
  fail 'reinstaller does not enforce 0.8.1'
